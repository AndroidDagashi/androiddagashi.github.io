...
6. **CI に lint を追加する（本 PR に同梱）**
   - `.github/workflows/build.yml` の install 後に `yarn site:lint` ステップを追加。PR / 非 main push で lint gate が効くようになり、以後「CI は green だが lint が壊れている」状態を検知できる。
   - deploy.yml はマージ後の本番デプロイ専用なので、lint gate は build.yml 側（PR CI）で担保する。
...
- **Renovate グルーピング刷新**: `renovate.json` の packageRules に `ESLint` グループを新設し、`eslint` / `@nuxt/eslint` / `@nuxt/eslint-config` / `@nuxt/eslint-plugin` / `eslint-config-prettier` / `eslint-plugin-prettier` を同グループに集約。さらに `separateMajorMinor: false` と `separateMultipleMajor: false` を指定し、**major と minor/patch を同一 PR に強制同居**させる。これにより eslint v10 のような peer 依存を巻き込む major 更新が来ても、`@nuxt/eslint` の互換バージョン bump が同じ PR で提案され、今回のような peer 不整合 PR が作られることを防ぐ。
- `all-minor-patch` 群と `Nuxt.js` 群からは eslint エコシステムを除外し、新設の `ESLint` 群へ集中。
